...
# Get certificate chain for catfact.ninja
# echo -n | openssl s_client -showcerts -connect catfact.ninja:443 2>/dev/null | awk '/-----BEGIN CERTIFICATE-----/,/-----END CERTIFICATE-----/' > $CERTS_DIR/catfact.ninja.pem

# Get certificate chain for entose.eu
echo -n | openssl s_client -showcerts -connect entsoe.eu:443 2>/dev/null | awk '/-----BEGIN CERTIFICATE-----/,/-----END CERTIFICATE-----/' > $CERTS_DIR/entsoe.eu.pem
